 ...
 */
 ...
 class Block_Styles_Scripts {
	...
	public function __construct() {
		 ...
		 */
		$this->prefix = 'fwd-';
		$this->directory = plugin_dir_path( __DIR__ ) . 'src/blocks/';
		...
		foreach ( $this->plugin_blocks as $block_name ) {
			register_block_type( plugin_dir_path( __DIR__ ) . 'build/blocks/' . $block_name );

			/**
			 * Require files for blocks that have additional functionality filters
			 */
			$file = plugin_dir_path( __DIR__ ) . 'src/blocks/' . $block_name . '/filters.php';
			if ( file_exists($file) ) {
				require( $file );
			}
		}
	...
	public function enqueue_editor_scripts_styles() {
		foreach ( $this->plugin_blocks as $block_name ) {
			$plugin_url  = plugin_dir_url( __DIR__ ) . 'build/blocks/' . $block_name;
			$file = plugin_dir_path( __DIR__ ) . 'src/blocks/' . $block_name . '/variations.js';
			if ( !file_exists($file) ) {
				continue;
			}

			wp_enqueue_script(
				$this->prefix . $block_name . '-variations-editor-scripts',
				$plugin_url . '/index.js',
				array('react', 'wp-block-editor', 'wp-components', 'wp-data', 'wp-hooks', 'wp-i18n')
			);
		}
	}
	...
	public function enqueue_frontend_scripts_styles() {
		...
		foreach ( $this->plugin_blocks as $block_name ) {
			if ( !in_array( $block_name, $page_blocks ) ) {
				continue;
			}

			$plugin_url = plugin_dir_url( __DIR__ ) . 'build/blocks/' . $block_name;
			wp_register_style(
				$this->prefix . $block_name . '-variations-styles',
				$plugin_url . '/style-index.css',
			);
			wp_enqueue_style( $this->prefix . $block_name . '-variations-styles' );
			wp_enqueue_script(
				$this->prefix . $block_name . '-variations-scripts',
				$plugin_url . '/view.js',
